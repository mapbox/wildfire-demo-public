  ...
  return Promise.all(promises).then((results) => {
    ...
    try {
      ...
      if (priorPerimeters.length === 0 && currentPerimeters.length !== 0) {
        return addToRelatedDataset(config, inciwebid, sourcePerimeterToGeoJson(inciwebid, currentPerimeters[0]));
      }
      ...
      culledPerimeters = cullRawPerimeters(allSortedPerimeters, BASE_CULLING_THRESHOLD);
    } catch (e) {
      console.log(`${inciwebid}: Failed`);
      throw e;
    }
    ...
    externalQueue.defer((done) => {
      ...
      retryRequest(url, (err, response, body) => {
        ...
        try {
          ...
          console.log(`${inciwebid}: Failed to parse prior perimeter response`);
          console.log(body);
          return reject(e);
        }
...
function addToRelatedDataset(config, inciwebid, perimeter) {
  return new Promise((resolve, reject) => {
    mapboxQueue.defer((done) => {
      ...
      function doTheAdding(datasets) {
        const datasetToDelete = `${config.perimeterDatasetPrefix || ''}${inciwebid}-perimeters`;
        const relatedDataset = datasets.find((dataset) => dataset.name === datasetToDelete);
        ...
        if (relatedDataset) {
          ...
          retryMapboxClient(config.mapboxClient, 'insertFeature', [perimeter, relatedDataset.id], (err) => {
            ...
            if (err) {
              console.log(`${inciwebid}: Failed to add perimeter to existing perimeter dataset`);
            } else {
...
function deleteRelatedDatasets(config, inciwebid) {
  return new Promise((resolve, reject) => {
    ...
    mapboxQueue.defer((done) => {
      if (existingDatasets) {
        doTheDeletion(existingDatasets);
      } else {
        retryMapboxClient(config.mapboxClient, 'listDatasets', [], (err, datasets) => {
          if (err) {
            console.log(`${inciwebid}: Failed to list datasets`);
      ...
      function doTheDeletion(datasets) {
        done();
        const datasetToDelete = `${config.perimeterDatasetPrefix || ''}${inciwebid}-perimeters`;
        try {
          const priorDatasets = datasets.filter((dataset) => dataset.name === datasetToDelete);
          if (!priorDatasets) return resolve();
          const promises = priorDatasets.map((priorDataset) => {
            console.log(`${inciwebid}: Deleting dataset name "${datasetToDelete}", id "${priorDataset.id}"`);
            return deleteDataset(priorDataset.id, config.mapboxClient);
          });
          return resolve(Promise.all(promises));
        } catch (e) {
          console.log(`${inciwebid}: Failed to delete datasets "${datasetToDelete}"`);
          resolve();
        }
      }
    });
  ...
  return new Promise((resolve) => {
    mapboxQueue.defer((done) => {
      retryMapboxClient(mapboxClient, 'deleteDataset', [datasetId], (err) => {
        if (err) console.log(`Failed to delete dataset with id "${datasetId}"`);
        done();
        resolve();
      });
    });
