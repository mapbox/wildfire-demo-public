...
const PERIMETER_API_BASE = 'https://wildfire.cr.usgs.gov/arcgis/rest/services/geomac_dyn/MapServer/1/query?';
...
module.exports = function (inciwebid, options) {
  ...
  return prudentRequest(url).then((data) => {
    if (data.response.statusCode !== 200) throw new Error(data.response.statusMessage);
